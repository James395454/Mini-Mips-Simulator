#include "MUL.h"   //include header file
using namespace std;
...
void MUL::operate()   //perform multiplication
{
	out = in1 * in2;
}
int MUL::result()   //return result
{
	return out;
}
void MUL::setvalues(int a,int b)
{
	in1 = a;
	in2 = b;
}
string MUL::writeoperands(ifstream & f, InstructionMemory & I_Mem)  //write 3 operands
{
	outs = "";
	in11 = "";
	in22 = "";
	...
	string addtomem = "MUL ";addtomem += in11;addtomem += ",";addtomem += in22;addtomem += ",";addtomem += outs;
	if (!I_Mem.find(addtomem))
	I_Mem.writetomemory(addtomem);
	return outs;
}
void MUL::writeinputs(DataMemory &D_Mem)  //write integer values of variables
{
